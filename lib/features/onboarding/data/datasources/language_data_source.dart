  ...
  Future<List<LanguageModel>> getLanguages() async {
    try {
      ...
      if (kDebugMode && languagesJson.isNotEmpty) {
        ...
        _logger.i('LanguageDataSource: First item content: ${languagesJson.first}');
      }
  ...
  Future<bool> saveUserLanguages(List<int> languageIds) async {
    try {
      ...
      if (languageIds.isEmpty) {
        _logger.w('LanguageDataSource: Attempted to save empty language array, ignoring request');
        return false;
      }
  ...
  Future<List<LanguageModel>> getUserLanguages() async {
    try {
      ...
      if (_cachedUserLanguages != null && !_shouldRefreshCache) {
        _logger.i('LanguageDataSource: Returning cached user languages (${_cachedUserLanguages!.length})');
      ...
      while (retryCount < maxRetries) {
        try {
          ...
          // The response format is { data: [...] }
          final responseData = response.data as Map<String, dynamic>;
          final userLanguagesJson = responseData['data'] as List<dynamic>? ?? [];
          ...
          _logger.i('LanguageDataSource: Parsed ${languages.length} language models');
