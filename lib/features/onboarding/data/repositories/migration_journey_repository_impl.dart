...
class MigrationJourneyRepositoryImpl implements MigrationJourneyRepository {
  ...
  Future<List<MigrationStep>> getMigrationSteps() async {
    try {
      ...
      if (!response.isSuccess || response.data == null) {
        _logger.w('No migration steps found',
            tag: 'MigrationJourneyRepository');
        return [];
      }
      ...
      if (!responseData.containsKey('data')) {
        _logger.w('Data field not found in response',
            tag: 'MigrationJourneyRepository');
        return [];
      }
      ...
      if (data is List<dynamic>) {
        _logger.i('Found ${data.length} migration steps (direct list)',
            tag: 'MigrationJourneyRepository');

        // Convert to models
        final steps =
            data.map((json) => MigrationStepModel.fromJson(json)).toList();
      ...
      // For backward compatibility with user-profile endpoint
          ...
          data.containsKey('migrationSteps')) {
        final List<dynamic> stepsJson = data['migrationSteps'] as List<dynamic>;

        _logger.i(
            'Found ${stepsJson.length} migration steps (nested in migrationSteps)',
            tag: 'MigrationJourneyRepository');

        // Convert to models
        final steps =
            stepsJson.map((json) => MigrationStepModel.fromJson(json)).toList();
      ...
      _logger.w('Migration steps not found in response format',
          tag: 'MigrationJourneyRepository');
      return [];
    } catch (e, stackTrace) {
      _logger.e(
  ...
  Future<bool> saveMigrationSteps(List<MigrationStep> steps,
    ...
    try {
      ...
      if (_areStepsUnchanged(steps)) {
        _logger.i('Steps unchanged, skipping save',
            tag: 'MigrationJourneyRepository');
        return true;
      }
      ...
      try {
        ...
        _logger.i(
            'Found birth country step in current steps: ${birthCountryStep.countryName}',
            tag: 'MigrationJourneyRepository');
      } catch (_) {
      ...
      if (birthCountryStep == null && _lastSavedSteps != null) {
        try {
          ...
          _logger.w(
              'Added birth country step back from cache: ${savedBirthCountryStep.countryName}',
              tag: 'MigrationJourneyRepository');
        } catch (_) {
      ...
      final processedSteps = steps.map((step) {
        ...
        final stepData = <String, dynamic>{
          ...
          'id': isBirthCountry
              ? step.id
              : (int.tryParse(step.id) != null ? int.parse(step.id) : step.id),
        ...
        if (isBirthCountry) {
          ...
          stepData['IsBirthCountry'] =
              true; // Add both formats for compatibility
          _logger.i('Flagged birth country step for ${step.countryName}',
              tag: 'MigrationJourneyRepository');
        }
      ...
      if (deletedSteps != null && deletedSteps.isNotEmpty) {
        ...
        _logger.w(
            '[$timestamp] 🗑️ Processing ${deletedSteps.length} deleted steps',
            tag: 'MigrationJourneyRepository');
      ...
      _logger.i('Migration steps saved successfully',
          tag: 'MigrationJourneyRepository');
      ...
      if (response.data != null) {
        try {
          ...
          if (savedSteps.isNotEmpty) {
            _logger.i('Received ${savedSteps.length} saved steps from server',
                tag: 'MigrationJourneyRepository');
            ...
            for (var i = 0; i < steps.length; i++) {
              ...
              try {
                ...
                final serverId =
                    matchingServerStep['Id'] ?? matchingServerStep['id'];
                if (serverId != null) {
                  _logger.d(
                      'Updating client step ID from ${clientStep.id} to $serverId',
                      tag: 'MigrationJourneyRepository');
                  ...
                  if (_lastSavedSteps != null) {
                    try {
                      ...
                      final updatedStep =
                          savedStep.copyWith(id: serverId.toString());
  ...
  bool _areStepsUnchanged(List<MigrationStep> steps) {
    ...
    for (var i = 0; i < steps.length; i++) {
      ...
      // Compare dates
      if ((newStep.startDate?.toIso8601String() !=
              oldStep.startDate?.toIso8601String()) ||
          (newStep.endDate?.toIso8601String() !=
              oldStep.endDate?.toIso8601String())) {
        return false;
      }
    }
  ...
  @override
  Future<List<MigrationStep>> updateMigrationStep(
      String id, MigrationStep step) async {
    try {
      _logger.i(
      ...
      if (index == -1) {
        index = currentSteps.indexWhere((s) =>
            s.countryId == step.countryId && s.visaTypeId == step.visaTypeId);
      }
  ...
  Future<List<MigrationStep>> removeMigrationStep(String id) async {
    try {
      ...
      if (birthCountryStep != null &&
          !finalSteps.any((step) => step.id.startsWith('birth_'))) {
        _logger.w(
