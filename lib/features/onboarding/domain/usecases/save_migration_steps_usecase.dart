...
class SaveMigrationStepsUseCase {
  ...
  Future<bool> call(List<MigrationStep> steps, {List<MigrationStep>? deletedSteps}) async {
    try {
      _logger.i('Saving ${steps.length} migration steps', tag: 'SaveMigrationStepsUseCase');
      ...
      final result = await _repository.saveMigrationSteps(orderedSteps, deletedSteps: deletedSteps);
      _logger.i('Migration steps saved successfully: $result', tag: 'SaveMigrationStepsUseCase');
      return result;
    } catch (e, stackTrace) {
      _logger.e(
  ...
  List<MigrationStep> _ensureSingleCurrentLocation(List<MigrationStep> steps) {
    ...
    final currentLocationCount = steps.where((step) => step.isCurrentLocation).length;
