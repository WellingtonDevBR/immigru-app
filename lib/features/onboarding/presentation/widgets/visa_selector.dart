...
class _VisaSelectorState extends State<VisaSelector> {
  ...
  void initState() {
    ...
    _showSelector = _selectedVisa == null; // Show selector if no visa is selected
  ...
  void _findAndSelectVisaById(int visaId) {
    ...
    try {
      ...
      if (matchingVisas.isNotEmpty) {
        ...
        print('Could not find visa with ID $visaId in available visas. Available visa IDs: ${_visas.map((v) => v.id).join(', ')}');
  ...
  Future<void> _loadVisas() async {
    ...
    try {
      ...
      List<Visa> visas;
      if (widget.countryId == 14) { // Australia
        ...
        visas = [
          ...
          Visa(
            ...
            type: 'Permanent', 
            description: 'For skilled workers who are nominated by a state or territory government.',
            pathwayToPR: true,
            allowsWork: true,
          ),
          Visa(
