...
class MigrationJourneyBloc
    extends Bloc<MigrationJourneyEvent, MigrationJourneyState> {
  ...
  List<MigrationStep> _ensureOnlyMostRecentIsCurrent(
    ...
    final targetCountries =
        updatedSteps.where((step) => step.isTargetCountry).toList();
    ...
    if (updatedSteps.isNotEmpty) {
      ...
      for (int i = 1; i < updatedSteps.length; i++) {
        ...
        final nextStep = updatedSteps[i -
            1]; // The next chronological step (remember list is in reverse chronological order)
  ...
  Future<void> _onStepAdded(
    ...
    try {
      ...
      if (isBirthCountryStep) {
        ...
        final hasBirthCountryStep =
            state.steps.any((step) => step.id.startsWith('birth_'));
  ...
  Future<List<MigrationStep>> _updateBirthCountryStep(
      MigrationStep newBirthCountryStep) async {
    // Get the current steps
    final currentSteps = List<MigrationStep>.from(state.steps);

    // Find the index of the birth country step
    final index =
        currentSteps.indexWhere((step) => step.id.startsWith('birth_'));
  ...
  Future<void> _onStepUpdated(
    ...
    try {
      ...
      final updatedSteps =
          await _updateMigrationStepUseCase(event.id, event.step);
      ...
      // Check if birth country step is missing from updated steps
      if (birthCountryStep != null &&
          !updatedSteps.any((step) => step.id.startsWith('birth_'))) {
        _logger.w(
  ...
  Future<void> _onStepRemoved(
    ...
    try {
      ...
      if (event.id.startsWith('birth_')) {
        ...
        emit(state.copyWith(
          errorMessage:
              'Birth country cannot be removed from your migration journey.',
          isLoading: false,
        ));
        return;
      }
      ...
      try {
        ...
        if (birthCountryStep != null &&
          ...
          if (!finalSteps.any((step) => step.id.startsWith('birth_'))) {
            ...
            emit(state.copyWith(
              steps:
                  _sortStepsByDate(forcedSteps), // Sort to ensure proper order
              isLoading: false,
              hasChanges: true,
