...
class UserProfileRemoteDataSourceImpl implements UserProfileRemoteDataSource {
  ...
  Future<UserProfileModel> getUserProfile(String userId) async {
    try {
      ...
      final userData =
          await _supabaseClient.from('User').select().eq('Id', userId).single();
      ...
      throw ErrorHandler.instance
          .handleException(e, tag: 'UserProfileRemoteDataSource');
    }
  }
  ...
  Future<String> uploadAvatar(String userId, String filePath) async {
    try {
      ...
      if (currentUser == null || currentUser.id != userId) {
        throw Exception(
            'Permission denied: Cannot upload avatar for another user');
      }
      ...
      final storagePath =
          '${StorageConfig.userPaths.avatarPath(userId)}/$fileName';

      // Upload the file to Supabase storage
      await _supabaseClient.storage.from(StorageConfig.buckets.users).upload(
          storagePath, file,
      ...
      await _supabaseClient
          .from('UserProfile')
          .update({'AvatarUrl': fileName}).eq('UserId', userId);
      ...
      logger.e('Error uploading avatar: ${e.toString()}',
  ...
  Future<String> uploadCoverImage(String userId, String filePath) async {
    try {
      ...
      if (currentUser == null || currentUser.id != userId) {
        throw Exception(
            'Permission denied: Cannot upload cover image for another user');
      }
      ...
      // Generate the storage path using the config
      final storagePath =
          '${StorageConfig.userPaths.coverPath(userId)}/$fileName';
      ...
      await _supabaseClient
          .from('UserProfile')
          .update({'CoverImageUrl': fileName}).eq('UserId', userId);

      // Get the public URL for immediate return
      final coverUrl = _supabaseClient.storage
          .from(StorageConfig.buckets.users)
          .getPublicUrl(storagePath);
  ...
  Future<bool> removeCoverImage(String userId) async {
    try {
      ...
      if (currentUser == null || currentUser.id != userId) {
        throw Exception(
            'Permission denied: Cannot remove cover image for another user');
      }
      ...
      if (fileName == null || fileName.isEmpty) {
        await _supabaseClient
            .from('UserProfile')
            .update({'CoverImageUrl': ''}).eq('UserId', userId);
        return true;
      }
      ...
      // Log the error using the appropriate logger
      final logger = UnifiedLogger();
      logger.e('Error removing cover image: ${e.toString()}',
          tag: 'UserProfileRemoteDataSource');
  ...
  Future<Map<String, int>> getUserStats(String userId) async {
    try {
      final logger = UnifiedLogger();
      logger.d('Fetching user stats for userId: $userId',
          tag: 'UserProfileRemoteDataSource');

      // Fetch active posts for this user (not deleted)
      final postsQuery = await _supabaseClient
          .from('Post')
          .select()
          .eq('UserId', userId)
      ...
      final followingQuery = await _supabaseClient
          .from('UserConnection')
          .select()
          .eq('SenderId', userId)
          .eq('Status', 'accepted');
      ...
      final followersCount = followersQuery.length;
      final followingCount = followingQuery.length;

      logger.d(
          'User stats fetched - posts: $postsCount, followers: $followersCount, following: $followingCount',
          tag: 'UserProfileRemoteDataSource');
      ...
      final logger = UnifiedLogger();
      logger.e('Error fetching user stats: ${e.toString()}',
          tag: 'UserProfileRemoteDataSource');
      throw ErrorHandler.instance
          .handleException(e, tag: 'UserProfileRemoteDataSource');
    }
